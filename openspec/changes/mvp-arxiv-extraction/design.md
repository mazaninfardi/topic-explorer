...
- **Term definitions grounded by snippet, not the PDF.** `POST /api/define { term, snippet }` → `{ text, terms }`: a general definition + one line using the snippet. Keeps the BFF stateless and avoids re-sending the paper per click.
